...
function emitRoom(room: Room, idOf: (id: string) => string, typeOf: (id: string) => string): string {
  const type = (room.type || 'object').trim() || 'object';
  const lines: string[] = [`${type} ${idOf(room.id)} {`];
  // Parent reference is typed with the PARENT object's class (e.g. `room parent = bar;`).
  if (room.parent) lines.push(`    ${typeOf(room.parent)} parent = ${idOf(room.parent)};`);
...
export const beguileExporter: Exporter = {
  ...
  export(world: World): string {
    ...
    const typeOf = (id: string) => {
      const t = (world.rooms.find(r => r.id === id)?.type || 'object').trim();
      return t || 'object';
    };

    const header = `// ${world.name} — generated by diorama\n// Target: Beguile (github.com/onyxring/beguiler)\n`;
    const body = world.rooms.map(r => emitRoom(r, idOf, typeOf)).join('\n\n');
    return `${header}\n${body}\n`;
  },
};
